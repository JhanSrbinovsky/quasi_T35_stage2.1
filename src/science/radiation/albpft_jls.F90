...
SUBROUTINE albpft (p_field,land_field,                                        &
                       land_index,surft_index,surft_pts,ilayers,              &
                       getprofile, albpft_call,                               &
                       albudir,albudif,cosz,lai_physical,alb_type,            &
...
!                                  ! include the surface type.
REAL, INTENT(IN) ::                                                           &
 albudir(land_field,2,npft)                                                   &
                             ! Direct albedo of the underlying
!                            ! surface.
,albudif(land_field,2,npft)                                                   &
                             ! Diffuse albedo of the underlying
!                            ! surface.
,cosz(p_field)                                                                &
                             ! Cosine of the zenith angle.
,lai_physical(land_field,npft) ! Actual leaf area index.
...
!$OMP                     u2, u3)                                          &
!$OMP SHARED(surft_pts, surft_index, land_index, albpft_call, band, omega, &
!$OMP        albobs_scaling_surft, alpar, omnir, alnir, omegal, alparl, n, &
!$OMP        omnirl, alnirl, omegau, omniru, alparu, orient, getprofile,   &
!$OMP        alb_type, cosz, lai, albudif, ilayers, fapar_dir, fapar_dif,  &
!$OMP        fapar_dir2dir, fapar_dir2dif, fapar_dif2dif, alniru, albudir, &
!$OMP        can_rad_mod, can_struct_a, fsun, l_niso_direct)               &
!$OMP             FIRSTPRIVATE(alpl, om) SCHEDULE(STATIC)
    DO j=1,surft_pts(n)
      l = surft_index(j,n)
      i = land_index(l)
...
! Firstly apply the scaling for this point to the inputs:
        IF (band == 1) THEN
          om(1) = omega(n) * albobs_scaling_surft(l,n,1)
          alpl(1) = alpar(n) * albobs_scaling_surft(l,n,1)
        ELSE IF (band == 2) THEN
          om(2) = omnir(n) * albobs_scaling_surft(l,n,2)
          alpl(2) = alnir(n) * albobs_scaling_surft(l,n,2)
        END IF
...
!           Use the full non-isotropic expression for the albedo
!           Asymmetry
            salb = (4.0/9.0) * (om(band) - 2.0 * alpl(band)) / om(band)
!           Albedo
            salb = 0.5 * om(band) * ( (1.0 + 3.0 * salb *  cosz(i)**2) *      &
                   ( 1.0 - cosz(i)*LOG((cosz(i)+1.0)/cosz(i)) ) -             &
                   1.5 * salb * cosz(i) )
          ELSE
...
!
! With these options fapar_dir represents only the diffuse PAR at each 
! level normalized by the incident PAR in the case when the incident 
! flux is entirely direct. fapar_dif is the normalized PAR at
! each level when the incident flux is entirely diffuse.
!-----------------------------------------------------------------------
