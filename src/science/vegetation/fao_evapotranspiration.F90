...
!-----------------------------------------------------------------------------
! Description:
!   Utilities for calculating FAO Penman-Monteith evapotranspiration for a reference crop,
!   as described in "Crop evapotranspiration - Guidelines for computing crop water 
!   requirements - FAO Irrigation and Drainage paper 56".
!   Note that the approximations used in this module are taken from this paper, 
!   and may be different to those used elsewhere in JULES.
!
! Current Code Owner: Karina Williams
!
! Code Description:
!   Language: Fortran 90.
...
!-----------------------------------------------------------------------------
  ...
  !
  ! Method:
  !   Use the Penman-Monteith equation for a reference crop, which is defined as having a
  ...
  !    * temperature and humidity at 2m above ground can be approximated by t1p5m and q1p5m. 
  !    * u and v components of wind at 10m above ground can be approximated by u10m and v10m
  ...
  ! Subroutine arguments
    ...
    REAL, INTENT(IN) ::                                          &
      ...
      trad(land_pts)                               
        ! temperature to use when calculating upward longwave
  ...
  !-----------------------------------------------------------------------------
    ...
    delta = 4098.0 * (0.6108 * EXP(17.27 * temperature_degC / (temperature_degC + 237.3))) &
            / denom

  END FUNCTION fao_slope_vapour_pressure_curve

  FUNCTION fao_vapour_pressure_deficit(t2m_degC, relhumidity_frac) RESULT (es_minus_ea)
  !-----------------------------------------------------------------------------
  ! Description:
  !   Calculates the vapour pressure deficit at 2m in kPa using approximations in
  !   FAO I&D paper 56. 
  !
  ! Current Code Owner: Karina Williams
  !
  ...
  !-----------------------------------------------------------------------------
    ...
    ! and lambda = 2.45 MJ kg-1 is latent heat of vaporization for an air temp of
    ! ~20 degree C (lambda only varies slightly over normal temperature ranges)
