...
,frac_a_loc(land_pts)                                                         &
!                           ! WORK Local copy of disturbance mask
!                           ! to allow the fix (including it)
!                           ! to be turned off
,frac(land_pts,ntype)                                                         &
...
DO t=1,trif_pts
  ...
  DO n=1,nnpft
    dfrac(l,n) = 0.0
  END DO
  space(l) = 1-nosoil(l)-frac_min*(nnpft-1)
...
DO t=1,trif_pts
  ...
  ELSE IF (frac(l,n) >                                                        &
      (space(l)-(1.0-crop(n))*frac_a_loc(l))) THEN
          dfrac(l,n) = dfrac(l,n)+                                            &
            ((space(l)-(1.0-crop(n))*frac_a_loc(l))-frac(l,n))
          frac(l,n) = space(l)-(1.0-crop(n))*frac_a_loc(l)
  END IF
  ...
  ELSE IF (frac(l,m) >                                                        &
      (space(l)-(1.0-crop(n))*frac_a_loc(l))) THEN
          dfrac(l,m) = dfrac(l,m)+((space(l)                                  &
      -(1.0-crop(m))*frac_a_loc(l))-frac(l,m))
          frac(l,m) = space(l)-(1.0-crop(n))*frac_a_loc(l)
  END IF
...
DO t=1,trif_pts
  ...
  ELSE IF (frac(l,n) >                                                        &
     (space(l)-(1.0-crop(n))*frac_a_loc(l))) THEN
          dfrac(l,n) = dfrac(l,n)+((space(l)                                  &
     -(1.0-crop(n))*frac_a_loc(l))-frac(l,n))
          frac(l,n) = space(l)-(1.0-crop(n))*frac_a_loc(l)
  END IF
