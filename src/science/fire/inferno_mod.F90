...
IF (ignition_method == 1) THEN
  nat_ign_l   = 2.7/s_in_month/m2_in_km2/12.0*0.75
    ! Prescribed data in  Flashes / km2 / day
    ! We want :           Flashes /  m2 / s
...
Z_l       =  a * (TsbyT_l - 1.) + b * LOG10(TsbyT_l)                          &
           + c * (10.0**( d * (1. - TsbyT_l)) - 1.)                           &
           + f * (10.0**( h * (TsbyT_l - 1.)) - 1.)
...
REAL  ,    INTENT(OUT)    ::                                                  &
  burnt_area_i_l
    ! The burnt area (fraction of gridbox per s)
...
DO l = 1, land_pts
  emitted_carbon_i(l) = MAX(burnt_area_i(l) *                                 &
    (  leaf_i(l)*(ccleaf_min_i+(ccleaf_max_i-ccleaf_min_i) *                  &
      (1.0-sm(l)))                                                            &
     + wood_i(l)*(ccwood_min_i+(ccwood_max_i-ccwood_min_i) *                  &
      (1.0-sm(l)))), 0.0)
...
SUBROUTINE calc_emitted_carbon_soil(                                          &
  ! Array INTENT(IN)
  land_pts, burnt_area, dpm_fuel, rpm_fuel, sm, dim_cs1,                      &
  ! Array INTENT(OUT)
...
! Current Code Owner: Stephane Mangeon, Imperial College and Met Office
!
! Code Description:
!   Language: Fortran 90
!
...
INTEGER                   :: land_pts, dim_cs1

REAL ,   INTENT(IN)       ::                                                  &
  burnt_area(land_pts),                                                       &
    ! Burnt Area fraction (s-1)
  dpm_fuel(land_pts),                                                         &
...
REAL ,   PARAMETER        ::                                                  &
  ccdpm_min = 0.8,                                                            &
  ccdpm_max = 1.0,                                                            &
    ! Damageable Plant Material burns between 80 to 100 %
  ccrpm_min = 0.0,                                                            &
  ccrpm_max = 0.2
...
DO l = 1,land_pts
  emitted_carbon_DPM(l) = MAX(burnt_area(l) *                                 &
    ( dpm_fuel(l)*(ccdpm_min+(ccdpm_max-ccdpm_min) *                          &
     (1.0-sm(l)))) ,0.0)
  emitted_carbon_RPM(l) = MAX(burnt_area(l) *                                 &
    ( rpm_fuel(l)*(ccrpm_min+(ccrpm_max-ccrpm_min) *                          &
     (1.0-sm(l)))) ,0.0)
END DO
