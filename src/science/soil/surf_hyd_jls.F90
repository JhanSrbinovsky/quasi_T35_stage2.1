...
SUBROUTINE surf_hyd (npnts,nsurft,surft_pts,surft_index,                      &
                     can_cpy,e_canopy,tile_frac,infil_surft,                  &
                     con_rain,ls_rain,con_rainfrac,ls_rainfrac,               &
                     melt_surft,snow_melt,                                    &
...
REAL                                                                          &
 can_wcnt(npnts,nsurft)!INOUT Tile canopy water contents (kg/m2).
...
!   DEPENDS ON: frunoff
    CALL frunoff (npnts,surft_pts(n),surft_index(:,n),frac_cov,               &
                can_cpy(:,n),can_cpy(:,n),infil_surft(:,n),                   &
...
! DEPENDS ON: sieve
    CALL sieve (npnts,surft_pts(n),surft_index(:,n),frac_cov,                 &
                can_cpy(:,n),can_cond,tile_frac(:,n),timestep,                &
                can_wcnt(:,n),tot_tfall)
! DEPENDS ON: frunoff
    CALL frunoff (npnts,surft_pts(n),surft_index(:,n),frac_cov,               &
                  can_cpy(:,n),can_wcnt(:,n),infil_surft(:,n),                &
                  can_cond,tile_frac(:,n),timestep,                           &
                  surf_roff)

! Canopy interception, throughfall and surface runoff for large-scale
! rain, assumed to cover "ls_rainfrac" of gridbox
! DEPENDS ON: sieve
    CALL sieve (npnts,surft_pts(n),surft_index(:,n),ls_rainfrac,              &
                can_cpy(:,n),ls_rain,tile_frac(:,n),timestep,                 &
                can_wcnt(:,n),tot_tfall)
! DEPENDS ON: frunoff
    CALL frunoff (npnts,surft_pts(n),surft_index(:,n),ls_rainfrac,            &
                  can_cpy(:,n),can_wcnt(:,n),infil_surft(:,n),                &
                  ls_rain,tile_frac(:,n),timestep,                            &
                  surf_roff)

! Canopy interception, throughfall and surface runoff for convective
! rain, assumed to cover fraction "con_rainfrac" of gridbox
! DEPENDS ON: sieve
    CALL sieve (npnts,surft_pts(n),surft_index(:,n),con_rainfrac,             &
                can_cpy(:,n),con_rain,tile_frac(:,n),timestep,                &
                can_wcnt(:,n),tot_tfall)
! DEPENDS ON: frunoff
    CALL frunoff (npnts,surft_pts(n),surft_index(:,n),con_rainfrac,           &
                  can_cpy(:,n),can_wcnt(:,n),infil_surft(:,n),                &
