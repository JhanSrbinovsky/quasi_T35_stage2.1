...
SUBROUTINE soil_htc (                                                         &
 npnts,nshyd,nsurft,soil_pts,soil_index,surft_pts,surft_index                 &
...
!      imported scalar parameters
   facur,GAMMA=>gamma_t,mmax,tacur,l_bedrock
...
,nsnow(npnts,nsurft)  ! IN Number of snow layerss
...
!                     ! WORK for Tmax
,hflux_base(npnts)
!                     ! WORK heat flux out of bottom soil layer.

REAL                                                                          &
sthu_b(npnts,nshyd)  ! WORK STHU at start of routine
  ...
  DO j=1,surft_pts(n)
    ...
    IF ( v_sat(i,1) > EPSILON(v_sat(i,1)) .AND.                               &
      ...
      IF ( snowdepth(i,n) <= 0.5*dz(1) ) THEN
        si_surft = 1. / ( 1. + 2.*snowdepth(i,n)/(dz(1) + dz(2)) )
      ELSE
        si_surft =(dz(1) + dz(2)) /                                           &
...
IF (l_bedrock) THEN
! DEPENDS ON: heat_con
    CALL heat_con (npnts,hcon(:,nshyd)                                        &
,                sthu(:,nshyd),sthf(:,nshyd)                                  &
...
!$OMP& tsl, smclu, smclsat, smclf, sthu_b, sthu_irr_gb, l_irrig_dmd)
...
!   add increment in sthu to sthu_irr_gb
        sthu_irr_gb(i,n) = sthu_irr_gb(i,n) + sthu(i,n) - sthu_b(i,n)
!   total soil moisture fraction should not exceed 100% or become negative
        IF ( sthu_irr_gb(i,n) + sthf(i,n) > 1.0 ) THEN
          sthu_irr_gb(i,n) = 1.0 - sthf(i,n)
        END IF
        sthu_irr_gb(i,n) = MAX( sthu_irr_gb(i,n), 0.0 )
      END IF
    END DO
  END DO
END DO
