...
SUBROUTINE sf_exch_cable(                                                          &
 ...
 bq_1,bt_1,canhc_surft,canopy,catch,dzsoil,dzsoil_elev,                       &
 flake,gc,hcons,                                                              &
 can_model,catch_snow, lq_mix_bl,                                             &
 ho2r2_orog,ice_fract_cat,snowdepth,snow_surft,pstar,qw_1,radnet,             &
 radnet_surft,sil_orog,tile_frac,timestep,                                    &
...
USE science_fixes_mod, ONLY: l_emis_ssi_full, l_fix_ctile_orog
...
,ws10                                                                         &
             ! 10-m wind speed (used in calculation of fresh snow
             ! density)
...
! DEPENDS ON: sf_resist
  CALL sf_resist (                                                            &
   ...
   snowdep_surft(:,n),vshr_land,fraca(:,n),resfs(:,n),resft(:,n),             &
   gc_irr_surft(:,n),resfs_irr_surft(:,n))
...
! DEPENDS ON: sf_rib
  CALL sf_rib (                                                               &
   ...
   tstar_surft(:,n),vshr_land,z0h_surft(:,n),z0m_surft(:,n),                  &
   z1_tq,z1_uv,                                                               &
...
! DEPENDS ON: fcdch
  CALL fcdch (                                                                &
   ...
   z0m_eff_surft(:,n),z0h_surft(:,n),zh,                                      &
...
!$OMP             SHARED(n, soil, surft_pts, surft_index, z0m_soil_in, z0h_soil, &
...
!  Input variables identical to main call except using different z0h
     ...
     surft_index(:,n),land_index,                                             &
     db_surft(:,n),vshr_land,                                                 &
     z0m_eff_surft(:,n),z0h_surft_classic(:,n),zh,                            &
...
! DEPENDS ON: sf_resist
  CALL sf_resist (                                                            &
   ...
   gc(:,n),snowdep_surft(:,n),vshr_land,fraca(:,n),                           &
   resfs(:,n),resft(:,n),                                                     &
   gc_irr_surft(:,n),resfs_irr_surft(:,n))
    ...
    IF ( cansnowtile(n) ) THEN
      DO k=1,surft_pts(n)
        ...
        rhokh_can(l,n) = rhostar(i,j) * cp /                                  &
                     (43. / (SQRT(cd_surft(l,n))*vshr_land(i,j)))
      END DO
    END IF
  END DO
!$OMP END PARALLEL DO
...
!$OMP PARALLEL DO IF(land_pts > 1) DEFAULT(NONE) PRIVATE(l)                   &
!$OMP SHARED(land_pts, hcon_lake, dzsoil, hcons, lake_h_snow_gb,              &
!$OMP        lake_h_ice_gb, snow_hcon, lake_depth_gb, nusselt_gb,             &
!$OMP        g_dt_gb, ts1_lake_gb, lake_t_snow_gb, lake_t_mxl_gb,             &
!$OMP        lake_t_ice_gb, lake_h_mxl_gb, lake_t_mean_gb, ts1)               &
!$OMP SCHEDULE(STATIC)
  DO l = 1, land_pts
!
        ...
        ELSE
          ...
          hcon_lake(l) = ( snow_hcon * lake_h_snow_gb(l)                      &
                          ...
                          +hcwat * lake_depth_gb(l)                           &
                                 * nusselt_gb(l)                              &
                          +hcons(l)*(dzsoil - lake_h_snow_gb(l)               &
                                            - lake_h_ice_gb( l)               &
                                            - lake_depth_gb( l)))             &
                        / dzsoil
    ...
    ! so that unadjusted tiles remain unaffected
      DO n=1,nsurft
        lw_down_elevcorr_surft(l,n) = lw_down_elevcorr_surft(l,n) -  &
          lw_down_surftsum*abs(lw_down_elevcorr_surft(l,n))/lw_down_surftabs
      END DO
    END IF
    ...
    DO n=1,nsurft
      IF (l_skyview) THEN
        radnet_surft(l,n)=radnet_surft(l,n) +                   &
           sky(i,j)*emis_surft(l,n)*lw_down_elevcorr_surft(l,n)
      ELSE
        radnet_surft(l,n)=radnet_surft(l,n) +                   &
...
!$OMP SHARED(land_pts, land_index, t_i_length, tsurf, ts1, t_elev, tl_1,    &
!$OMP        dzsurf, dzsoil, hcons_surf, hcons, canhc_surf, canhc_surft,    &
!$OMP        nsmax, nsnow, tsnow, ds, hcons_snow, cansnowtile,              &
!$OMP        snowdepth,                                                     &
!$OMP        l_snow_nocan_hc, l_flake_model, l_aggregate, lake,             &
!$OMP        n, ts1_lake_gb, hcon_lake, l_elev_land_ice, l_lice_point,      &
!$OMP        tsurf_elev_surft, dzsoil_elev)                                 &
!$OMP SCHEDULE(STATIC)
  DO l = 1,land_pts
    j=(land_index(l)-1)/t_i_length + 1
    i = land_index(l) - (j-1)*t_i_length
...
!$OMP END PARALLEL DO
  ...
  CALL sf_flux (                                                              &
   ...
   z0m_eff_surft(:,n),z1_tq,lh0,emis_surft(:,n),emis_soil,                    &
...
IF (l_ctile .AND. .NOT. l_fix_ctile_orog) THEN
  WHERE( (flandg(:,:) > 0.0) .AND. (flandg(:,:) < 1.0) )
    z1_tq_ctile(:,:)     = z1_tq(:,:) + z_land(:,:)
  END WHERE
ENDIF
IF (i_modiscopt == on) THEN
  ALLOCATE(z1_tq_top_ctile(t_i_length,t_j_length))
  z1_tq_top_ctile = z1_tq_top
  IF (l_ctile .AND. .NOT. l_fix_ctile_orog) THEN
    WHERE( (flandg(:,:) > 0.0) .AND. (flandg(:,:) < 1.0) )
      z1_tq_top_ctile(:,:) = z1_tq_top(:,:) + z_land(:,:)
    END WHERE
  ENDIF
ELSE
! Default allocation
...
! DEPENDS ON: sf_rib
    CALL sf_rib (                                                             &
     ...
     tstar_sice_cat(:,:,n),vshr_ssi,z0h_ice(:,:,n),z0m_ice(:,:,n),            &
     z1_tq_sea,z1_uv,                                                         &
     rib_ice(:,:,n),db_ice(:,:,n)                                             &
      )
  END DO
...
! DEPENDS ON: sf_rib
  CALL sf_rib (                                                               &
    ...
    tstar_sice_cat(:,:,1),vshr_ssi,z0h_ice(:,:,1),z0m_ice(:,:,1),             &
    z1_tq_sea,z1_uv,                                                          &
    rib_ice(:,:,1),db_ice(:,:,1)                                              &
     )
...
! Sea and sea ice surface drag coefficient
...
IF (nice_use > 1) THEN
  ...
  CALL sf_flux (                                                              &
     ...
     qs1,qstar_sea,qw_1,                                                      &
     radnet(:,:,1),array_one,rhokh_1_sea,array_false,array_zero,              &
...
CALL sf_flux (                                                                &
     ...
     array_zero_int,0,array_zero,dzssi,hcons_sea,                             &
     qs1,qstar_sea,qw_1,                                                      &
     radnet(:,:,1),array_one,rhokh_1_sice,array_false,array_zero,             &
     sea_frac,timestep,tl_1,tstar_sea,tstar_sea,                              &
     array_zero,array_zero,z0h_sea,z0msea,z1_tq_sea,lh0,                      &
     array_emis,array_one,                                                    &
     seasalinityfactor,array_zero,array_one,                                  &
...
IF (nice_use >  1) THEN
  DO n=1, nice_use
    CALL sf_flux (                                                            &
     ...
     qs1,qstar_ice_cat(:,:,n),qw_1,radnet(:,:,n),array_one,                   &
     rhokh_1_sice_ncats(:,:,n),array_false,array_zero,                        &
     sice_frac_ncat(:,n),timestep,tl_1,ti_cat(:,:,n),                         &
     tstar_sice_cat(:,:,n),                                                   &
     array_zero,array_zero,z0h_ice(:,:,n),z0m_ice(:,:,n),                     &
     z1_tq_sea,lh0,                                                           &
     array_emis,array_one,                                                    &
...
chr1p5m_sea(:,:) = 0.0
  CALL sfl_int (                                                              &
     ssi_pts,sea_pts,sea_index,ssi_index,fssi_ij,                             &
...
!
  ...
  DO n=1,nice_use
    CALL sfl_int (                                                            &
       ssi_pts,sice_pts_ncat(n),sice_index_ncat(:,n),ssi_index,               &
...
! Calculate gridbox mean chr1p5m for sea and sea ice
  chr1p5m_ssi_mean(:,:) = 0.0    ! Initialise
