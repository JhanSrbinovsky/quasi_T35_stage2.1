...
! URBAN-2T: Over-write anthrop_heat_surft for roof and canyon doing roof at same
! time as canyon to avoid over-writting land-ice. Capability is now here to
! do spatially varying anthrop_heat_surft.
  IF ( l_urban2t ) THEN
    n = urban_canyon
    DO k=1,surft_pts(n)
      l = surft_index(k,n)
! Here anthrop_heat_surft(urban_roof) is a fraction (anthrop_heat_scale) of
! anthrop_heat_surft(urban_canyon) where wrr_gb is the canyon fraction.
! The total from the urban tile is conserved.
