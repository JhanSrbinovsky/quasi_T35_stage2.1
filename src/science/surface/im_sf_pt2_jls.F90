...
#if defined(UM_JULES)
IF (l_endgame .AND. model_type /= mt_single_column) THEN
#else
IF (l_endgame) THEN
#endif
  offset=1
...
! Include leads.
         ...
         - gamma_1 * ( 1.0 - flandg(i,j) ) * rhokh1_sea(i,j)                  &
         * ( 1.0 - ice_fract(i,j) )
...
! Do not include leads - use original scheme.
          apart(i,j,1)= flandg(i,j)*apart(i,j,1)                              &
         - gamma_1 * ( 1.0 - flandg(i,j) ) * rhokh1_sice(i,j,1)               &
         * ( 1.0 - ice_fract(i,j) )
          apart(i,j,2)= flandg(i,j)*apart(i,j,2)
          bpart(i,j,1)= flandg(i,j)*bpart(i,j,1)
          bpart(i,j,2)= flandg(i,j)*bpart(i,j,2)                              &
...
!       in the numerator.
        dtstar_surft(l,n) = dtstar_surft(l,n) + gamma_1 *                     &
               ( cp * rhokh_1(l,n) *                                          &
                 ( dtl1_1(i,j) - ctctq1(i,j) * ftl_gb(i,j) ) +                &
                 lat_ht * resft(l,n) * rhokh_1(l,n) *                         &
                 ( dqw1_1(i,j) - ctctq1(i,j) * fqw_gb(i,j) ) ) /              &
