...
! Local variables
  REAL ::                                                                     &
    ...
    dtstar_ij(tdims%i_start:tdims%i_end,tdims%j_start:tdims%j_end,nice_use),  &
    ...
    con_rainfrac(t_i_length,t_j_length),                                      &
      !Convective rain fraction
    ls_rainfrac(t_i_length,t_j_length),                                       &
...
!-------------------------------------------------------------------------------
  ...
  LOGICAL, PARAMETER :: L_FLUX_BC = .FALSE.
    ...
    !- why is this in the surface scheme?!!!
...
IF ( l_cosz ) THEN
  CALL zenith( t_i_length*t_j_length, cosz_gb )
ELSE
...
bt_1(:,:) = 1. / tl_1_ij(:,:)
bq_1(:,:) = c_virtual / (1. + c_virtual*qw_1_ij(:,:))
zh(:,:)   = 1000.
...
!  epot_surft
!  fraca, rhokh, rhokh_surft, rhokh_sice, dtstar_ij_surft, dtstar_ij, z0hssi,
!  z0h_surft,
...
taux_1_ij(:,:)   =                                                            &
          flandg(:,:) * taux_land_ij(:,:) + (1.-flandg(:,:)) * taux_ssi_ij(:,:)
tauy_1_ij(:,:)   =                                                            &
          flandg(:,:) * tauy_land_ij(:,:) + (1.-flandg(:,:)) * tauy_ssi_ij(:,:)
...
CALL surf_couple_implicit(                                                    &
  ...
  rhokm_1, rhokm_1, gamma1, gamma2, alpha1, alpha1_sice,                      &
  ashtf_prime, ashtf_prime_surft, du, dv, fraca, resfs, resft, rhokh,         &
  rhokh_surft, rhokh_sice, rhokh_sea_ij, z0hssi, z0mssi, z0h_surft, z0m_surft,&
  chr1p5m, chr1p5m_sice, canhc_surft, flake, tile_frac, wt_ext_surft,         &
  l_flux_bc, cdr10m, cdr10m, cdr10m_n, cdr10m_n,                              &
  !Diagnostics, INTENT(INOUT)
  sf_diag,                                                                    &
  !Fluxes INTENT(INOUT)
  fqw_sicat, ftl_sicat, fqw_surft, fqw_1_ij, ftl_1_ij, ftl_surft,             &
  !Misc INTENT(INOUT)
  epot_surft, dtstar_ij_surft, dtstar_ij, radnet_sice, olr,                   &
  !Fluxes INTENT(OUT)
  tstar_ij, le_surft, radnet_surft, e_sea_ij, h_sea_ij, taux_1_ij, tauy_1_ij, &
  ecan_surft, ei_ij, esoil_ij, ext_gb, snowmelt_ij, melt_surft,               &
...
CALL surf_couple_implicit(                                                    &
  ...
  tstar_ij, le_surft, radnet_surft, e_sea_ij, h_sea_ij, taux_1_ij, tauy_1_ij, &
  ecan_surft, ei_ij,                                                          &
  esoil_ij, ext_gb, snowmelt_ij, melt_surft,                                  &
  ecan_ij, ei_surft, esoil_surft, sea_ice_htf_sicat, surf_ht_flux_ij,         &
  surf_htf_surft,                                                             &
  !Misc INTENT(OUT)
  error                                                                       &
...
CALL surf_couple_extra(                                                       &
  ...
  !Fluxes INTENT(IN)
  ei_surft, surf_htf_surft, ecan_surft, ext_gb, sw_surft,                     &
  !Misc INTENT(IN)
