...
!-----------------------------------------------------------------------------
    ...
    REAL, INTENT(OUT)    :: cosz(npoints)      ! Cosine of zenith angle
...
!-----------------------------------------------------------------------------
    ...
    DO l = 1,npoints
      ...
      hh = pi * (2.0 * REAL(time) / 86400.0 + longitude(i,j) / 180.0 - 1.0)
...
! The dependence of phot on day_of_year is via the solar declination angle sd
        dphotdt(l) = earth_tilt * pi_over_180 * 2.0 * 24.0 / REAL(days_in_yr) &
