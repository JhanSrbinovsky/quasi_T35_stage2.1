...
!   in TRIFFID.
!
! Current Code Owner: Anna Harper
!
! Code Description:
!   Language: Fortran 90.
...
!   Calculate the canopy-average leaf nitrogen.  
    IF( l_trait_phys ) THEN
      ...
      SELECT CASE ( can_rad_mod )
        CASE ( 4,5 )
          nl_ave = nmass(n) * lma(n) * (1. - EXP(-kn(n))) / kn(n)
        CASE DEFAULT
          x_tmp = kn(n)*lai_bal
    ...
    ELSE
      ...
      SELECT CASE ( can_rad_mod )
        CASE ( 4,5 )
          nl_ave = nl0(n) * sigl(n) * (1. - EXP(-kn(n))) / kn(n)
        CASE DEFAULT
          x_tmp = kn(n) * lai_bal
          IF ( x_tmp > EPSILON(0.0) ) THEN
...
!   Calculate leaf N, adjusting for phenology/labile pool.
    n_leaf = ( nl_ave * lai_bal * phen ) +                                    &
              ((1. - phen) * nl_ave * lai_bal * (1.+ retran_l(n))/2.)
...
!------------------------------------------------------------------------------
  IF ( l_trait_phys ) THEN
    n_root = nr(n) * root
    fstem  = 1./a_ws(n)
    n_stem = wood * ( (nsw(n) * fstem) + (1.-fstem) * nsw(n) * hw_sw(n))
  ELSE
