...
!-----------------------------------------------------------------------------
  ...
  USE c_r_cp, ONLY : cp
  USE c_g, ONLY : g
  USE planet_constants_mod, ONLY: one_minus_epsilon, grcp
...
! Constants for surface transfer coefficients
  ...
  REAL, PARAMETER ::                                                          &
    ...
    cnst_cndd_1 = 6.004e+02,                                                  &
        ! Constant in formula for convective gustiness
    cnst_cndd_2 = -4.375e+03,                                                 &
        ! Constant in formula for convective gustiness
    min_wind = 1.0e-3,                                                        &
...
! Other variables used in surface calculations
  REAL, PARAMETER ::                                                          &
    kn = 0.78
        ! Nitrogen Allocation coefficient: Tellus 59B, 553-565 (2007), page 555
...
! Switches
    ...
    l_elev_lw_down,l_elev_land_ice,                                           &
! Parameters
    orog_drag_param, hleaf, hwood, beta1, beta2, fwe_c3, fwe_c4,              &
    q10_soil, kaps, kaps_roth, sorp, n_inorg_turnover
...
! Verify that the integer switches have suitable values
    IF ( i_modiscopt < 0 .OR. i_modiscopt > 1 ) THEN
      CALL ereport("check_jules_surface", 101, "i_modiscopt should be 0 or 1")
    END IF

    IF ( all_tiles < 0 .OR. all_tiles > 1 ) THEN
      CALL ereport("check_jules_surface", 101, "all_tiles should be 0 or 1")
    END IF

    IF ( cor_mo_iter < 0 .OR. cor_mo_iter > Limit_ObukhovL ) THEN
      CALL ereport("check_jules_surface", 101,                                &
                   "cor_mo_iter should be 0, 1, 2 or 3")
    END IF

    IF ( iscrntdiag < 0 .OR. iscrntdiag > ip_scrndecpl2 ) THEN
      CALL ereport("check_jules_surface", 101,                                &
                   "iscrntdiag should be 0, 1, or 2")
    END IF
    ...
    IF ( fd_stab_dep < 0 .OR. fd_stab_dep > 1 ) THEN
      CALL ereport("check_jules_surface", 101, "fd_stab_dep should be 0 or 1")
    END IF
